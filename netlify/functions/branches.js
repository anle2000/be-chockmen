...
const { commonHeaders, getToken, getKiotVietToken, updateToken, project, retailer } = require("./shared");
        ...
        if (error.response && error.response.status === 401) {
            try {
                ...
                return {
                    statusCode: 500,
                    headers: commonHeaders,
                    body: JSON.stringify({ error: `Error while retrying: ${retryError?.message}` }),
                };
            }
        }
